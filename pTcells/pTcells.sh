...
#SBATCH --job-name=pTcells_pipeline
#SBATCH --output=pTcells_pipeline_outs/pTcells.out
#SBATCH --error=pTcells_pipeline_errs/pTcells.err
...
# R: /data/samanthafs/scratch/lab_mguell/dfragoso/R_libs /data/samanthafs/homes/aplic/haswell/software/R/4.5.1-gfbf-2023a/lib64/R/library
...
# Dataset / files
DATASET="Fachrul_2026"
WORK_DIR="/scratch/lab_mguell/dfragoso/scRNA-seq_datasets/${DATASET}"
...
# QC & filtering
NMADS_QC="3"        # <-- number of median absolute deviations (MADs) from
ONLY_NORMAL="TRUE"
...
# FILTER_CTS="T cell, B cell" # <-- key words you want the remaining cells to have
...
# Redirecting the .err and .out files
exec > "${WORK_DIR}/pTcells_${DATASET}.out"
exec 2> "${WORK_DIR}/pTcells_${DATASET}.err"


echo "[$(date +"%Y-%m-%d %H:%M:%S")] Job started on $(hostname)"


# STEP 1: Obtaining raw counts
...
echo "[$(date +"%Y-%m-%d %H:%M:%S")] Job finished on $(hostname)"
